...
def Distance_Two_Bodies(Body1, Body2):
...
def pair_correlation_function(distances, L, N):
    """ returns Pair Correlation Function g(r), and plots the result """
    ...
    # mask distances larger than L/2
    data = distances[distances < L/2.]
    
    num_bins = 100
    ###bin_edges = np.arange(0, num_bins, 1)
    ###bins = np.hstack([bin_edges[:-1].reshape(-1,1), bin_edges[1:].reshape(-1,1)])
    bins = np.linspace(1., L/2, num_bins)
    ###r = bins.sum(axis = 1) / 2
    dr = bins[1] - bins[0]
    r = np.linspace(dr/2, bins[-1] - dr/2, num_bins-1)
    ###dr = bin_edges[1] - bin_edges[0]
    n, _ = np.histogram(data, bins)
    V = L**3
    g = n * 2*V / (N*(N-1) * 4*np.pi * r**2 * dr)
...
def pressure_calc(dists, dUdr, T, rho, num_atoms):
    ...
    p = T*rho * (1 - 1/(3*num_atoms*T)*0.5*np.sum(dists*dUdr) )
...
def plotting_state(state, box_size):
    ...
    for p in np.rollaxis(state, 1):
        plt.scatter(p[0, 0], p[0, 1], s=10, c='k', zorder=-5)
...
def plotting_lambdas(lambdas):
    ...
    plt.xlabel("Time t")
    plt.ylabel("Lambda Scale Sactor")
    plt.show()
    plt.close()
...
def main():
    ...
    rescaling_interval = 2
    rescaling_time = rescaling_interval*h
    rescaling_end = 200 * rescaling_time
